...
function Level:update(dt)
    ...
    for k, alienextra in pairs(self.aliensextra) do
        local xPos, yPos = alienextra.body:getPosition()
        local xVel, yVel = alienextra.body:getLinearVelocity()

        if xPos < 0 or (math.abs(xVel) + math.abs(yVel) < 1.5) then
            alienextra.body:destroy()
            table.remove(self.aliensextra, k)
        end
    ...
    if love.keyboard.isDown('space') and self.aliensplit == false and self.collided == false 
    and self.launchMarker.launched then

        local AlienX = self.launchMarker.alien.body:getX()
        local AlienY = self.launchMarker.alien.body:getY()
        local AlienXVel, AlienYVel = self.launchMarker.alien.body:getLinearVelocity()

        local AlienExtraTop = Alien(self.world, 'round', AlienX, AlienY, 'Player')
        local AlienExtraBottom = Alien(self.world, 'round', AlienX, AlienY, 'Player')

        AlienExtraTop.body:setLinearVelocity(AlienXVel, AlienYVel - 45)
        AlienExtraBottom.body:setLinearVelocity(AlienXVel, AlienYVel + 45)

        table.insert(self.aliensextra, AlienExtraTop)
        table.insert(self.aliensextra, AlienExtraBottom)
